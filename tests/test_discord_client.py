...
class TestDiscordAPIMethods:
    ...
    async def test_get_current_user(self, discord_client):
        ...
        with patch.object(discord_client, 'get', return_value={"id": "123"}) as mock_get:
    ...
    async def test_get_user_guilds(self, discord_client):
        ...
        with patch.object(discord_client, 'get', return_value=[{"id": "456"}]) as mock_get:
    ...
    async def test_get_guild_channels(self, discord_client):
        ...
        with patch.object(discord_client, 'get', return_value=[{"id": "789"}]) as mock_get:
    ...
    async def test_get_channel_messages(self, discord_client):
        ...
        with patch.object(discord_client, 'get', return_value=[{"id": "msg1"}]) as mock_get:
            result = await discord_client.get_channel_messages("123", limit=10, before="456")
            mock_get.assert_called_once_with(
                "/channels/123/messages", 
                params={"limit": 10, "before": "456"}
            )
    ...
    async def test_send_message(self, discord_client):
        ...
        with patch.object(discord_client, 'post', return_value={"id": "msg123"}) as mock_post:
    ...
    async def test_send_dm(self, discord_client):
        ...
        with patch.object(discord_client, 'create_dm_channel', return_value={"id": "dm123"}) as mock_create_dm:
            with patch.object(discord_client, 'send_message', return_value={"id": "msg123"}) as mock_send:
