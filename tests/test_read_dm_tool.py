...
async def test_read_direct_messages_success(mock_server, mock_context):
    ...
    mock_discord_client.create_dm_channel.return_value = {
        "id": "987654321"
    }
    ...
    mock_discord_client.get_channel_messages.assert_called_once_with("987654321", limit=10)
...
async def test_read_direct_messages_dm_disabled(mock_server, mock_context):
    ...
    mock_discord_client.create_dm_channel.side_effect = DiscordAPIError("Cannot send messages to this user", 403)
    
    read_dm_tool = mock_server._tools["read_direct_messages"]
    result = await read_dm_tool(user_id="123456789", limit=10)
    
    assert "❌ Error: Cannot create DM channel with testuser#1234. User may have DMs disabled." in result
...
@pytest.mark.asyncio
async def test_read_direct_messages_with_attachments_and_embeds(mock_server, mock_context):
...
async def test_read_direct_messages_long_content_truncation(mock_server, mock_context):
    ...
    assert len([line for line in result.split('\n') if 'AAAA' in line][0]) < len(long_content)
...
async def test_read_direct_messages_unexpected_error(mock_server, mock_context):
    ...
    assert "❌ Unexpected error while reading direct messages: Unexpected error" in result
