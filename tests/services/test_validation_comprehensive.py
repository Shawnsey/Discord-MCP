...
def test_comprehensive_validation_workflow():
    ...
    assert (
        guild_result.is_valid
    ), f"Guild ID validation failed: {guild_result.error_message}"

    channel_result = validator._validate_discord_id(context.channel_id, "channel")
    assert (
        channel_result.is_valid
    ), f"Channel ID validation failed: {channel_result.error_message}"

    user_result = validator._validate_discord_id(context.user_id, "user")
    assert (
        user_result.is_valid
    ), f"User ID validation failed: {user_result.error_message}"
    ...
    ), f"Message content validation failed: {content_result.error_message}"
    ...
    error_response = validator._create_validation_error_response(
        invalid_id_result, context
    )
    expected_error = (
        "❌ Error in send_message: Invalid guild ID format. Discord IDs must be numeric"
    )
    ...
    perm_denied = validator._create_permission_denied_response(
        "channel", context.channel_id, "Bot lacks required permissions."
    )
    ...
    not_found = validator._create_not_found_response(
        "message", "111222333444555666", "Message may have been deleted."
    )
...
def test_validation_result_chaining():
    ...
    def validate_moderation_action(
        guild_id: str, user_id: str, duration: int, reason: str
    ):
        ...
        failed_validations = [
            (name, result) for name, result in results if not result.is_valid
        ]
    ...
    result = validate_moderation_action(
        "123456789012345678", "987654321098765432", 60, "Spamming in chat"
    )
    ...
    result = validate_moderation_action(
        "invalid_guild", "987654321098765432", 60, "Spamming in chat"
    )
