...
def test_validation_mixin():
    ...
    validation_result = ValidationResult.error(
        "Test error message", ValidationErrorType.INVALID_INPUT
    )
...
def test_message_content_validation_utilities():
    ...
    too_long_response = validator._create_message_content_too_long_response(
        2500, "message"
    )
    ...
    error = validator._validate_and_format_message_content_error(
