...
class TestModerationSetup:
    ...
    @pytest.mark.asyncio
    async def test_successful_moderation_setup(self, discord_service, mock_discord_client):
        """Test successful moderation setup with valid guild and user."""
        # Mock successful API responses
        guild_data = {"id": "123456789", "name": "Test Guild"}
        user_data = {"id": "987654321", "username": "testuser", "global_name": "Test User"}
    ...
    @pytest.mark.asyncio
    async def test_moderation_setup_guild_permission_denied(self, discord_service, mock_settings):
    ...
    @pytest.mark.asyncio
    async def test_moderation_setup_guild_not_found(self, discord_service, mock_discord_client):
    ...
    @pytest.mark.asyncio
    async def test_moderation_setup_user_not_found(self, discord_service, mock_discord_client):
    ...
    @pytest.mark.asyncio
    async def test_moderation_setup_unexpected_error(self, discord_service, mock_discord_client):
...
class TestModerationTargetValidation:
    ...
    @pytest.mark.asyncio
    async def test_successful_target_validation_with_membership(self, discord_service, mock_discord_client):
        ...
        mock_discord_client.get_guild_member.return_value = {"user": {"id": "987654321"}}
        
        with patch.object(discord_service, '_validate_role_hierarchy', return_value=None):
    ...
    @pytest.mark.asyncio
    async def test_target_validation_user_not_member_required(self, discord_service, mock_discord_client):
    ...
    @pytest.mark.asyncio
    async def test_target_validation_user_not_member_optional(self, discord_service, mock_discord_client):
    ...
    @pytest.mark.asyncio
    async def test_target_validation_hierarchy_failure(self, discord_service, mock_discord_client):
        ...
        with patch.object(discord_service, '_validate_role_hierarchy', return_value="Hierarchy error"):
            error = await discord_service._validate_moderation_target(
                "123456789", "987654321", setup_data, require_membership=True
            )
    ...
    @pytest.mark.asyncio
    async def test_target_validation_unexpected_error(self, discord_service, mock_discord_client):
...
class TestModerationLogging:
    ...
    def test_successful_moderation_logging(self, discord_service, mock_logger):
        ...
        discord_service._log_moderation_action(
            "timeout", setup_data, "123456789", "987654321", True,
            duration_minutes=10, reason="Test reason"
        )
    ...
    def test_failed_moderation_logging(self, discord_service, mock_logger):
        ...
        # Call the method
        discord_service._log_moderation_action(
            "kick", setup_data, "123456789", "987654321", False,
            reason="Test reason"
        )
...
class TestModerationResponseFormatting:
    ...
    def test_create_moderation_success_response(self, discord_service):
        ...
        response = discord_service._create_moderation_success_response(
            "timed out", setup_data, "123456789", "987654321",
            duration="10 minutes", reason="Test reason"
        )
    ...
    def test_create_moderation_success_response_no_additional_details(self, discord_service):
    ...
    def test_create_moderation_permission_error(self, discord_service):
        ...
        # Verify error format
        assert "❌ Error: Bot does not have 'moderate_members' permission in Test Guild." == error

    def test_create_moderation_hierarchy_error(self, discord_service):
        """Test creation of moderation hierarchy error."""
        error = discord_service._create_moderation_hierarchy_error(
            "kick", "Test Guild"
        )
        
        # Verify error format
        assert "❌ Error: Bot does not have permission to kick users in Test Guild. Role hierarchy may prevent this action." == error
...
class TestModerationFrameworkIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_moderation_workflow_success(self, discord_service, mock_discord_client):
        ...
        with patch.object(discord_service, '_validate_role_hierarchy', return_value=None):
            ...
            response = discord_service._create_moderation_success_response(
                "tested", setup_data, "123456789", "987654321",
    ...
    @pytest.mark.asyncio
    async def test_full_moderation_workflow_failure(self, discord_service, mock_discord_client):
