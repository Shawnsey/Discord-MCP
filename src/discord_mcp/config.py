...
class DiscordConfig(BaseSettings):
    ...
    allowed_guilds: Optional[List[str]] = Field(
        default=None,
        description="Comma-separated list of allowed guild IDs"
    )
    ...
    def parse_allowed_channels(cls, v):
        ...
        if v is None or v == "":
            return None
        if isinstance(v, str):
            return [channel_id.strip() for channel_id in v.split(',') if channel_id.strip()]
        return v
...
class Settings(BaseSettings):
    ...
    def get_allowed_guilds_list(self) -> Optional[List[str]]:
        ...
        if self.allowed_guilds is None:
            return None
        return [guild_id.strip() for guild_id in self.allowed_guilds.split(',') if guild_id.strip()]
    
    def get_allowed_channels_list(self) -> Optional[List[str]]:
        """Get allowed channels as a list."""
        if self.allowed_channels is None:
            return None
        return [channel_id.strip() for channel_id in self.allowed_channels.split(',') if channel_id.strip()]
...
def load_settings() -> Settings:
    ...
    try:
        ...
        logger.info(
            ...
            allowed_guilds_count=len(settings.get_allowed_guilds_list()) if settings.get_allowed_guilds_list() else None,
            allowed_channels_count=len(settings.get_allowed_channels_list()) if settings.get_allowed_channels_list() else None
        )
        return settings
