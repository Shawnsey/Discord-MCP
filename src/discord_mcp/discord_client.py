...
class DiscordClient:
    ...
    async def _handle_response(self, response: aiohttp.ClientResponse) -> Dict[str, Any]:
        ...
        if 400 <= response.status < 500:
            error_message = response_data.get('message', f'Client error: {response.status}')
            logger.error("Discord API client error", 
    ...
    async def _request(
        ...
        if not self.session:
            raise RuntimeError("Discord client not started. Use async context manager or call start().")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt == max_retries:
                    raise
                logger.info("Rate limited, retrying", retry_after=e.retry_after, attempt=attempt + 1)
                await asyncio.sleep(e.retry_after)
                
            except (ClientError, asyncio.TimeoutError) as e:
                if attempt == max_retries:
                    logger.error("Max retries exceeded", error=str(e))
                    raise DiscordAPIError(f"Network error after {max_retries} retries: {str(e)}")
